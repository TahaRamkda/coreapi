...
namespace WhatsAppAPISolutionBL.Master.Services
{
    public class FlowsService : IFlowsService
    {
        ...
        public async Task<List<UFlow>> GetFlowListAsync(int clientId, string searchStr = "", int pageNo = 0, int pageSize = int.MaxValue, int senderId = 0, string lang = "")
        {
            var startProcTime = DateTime.UtcNow;
            var response = await _dbContext2.Flow.FromSqlInterpolated($"exec usp_GetFlowList @ClientId={clientId}, @SearchStr={searchStr ?? ""}, @PageNo={pageNo}, @PageSize={pageSize}, @SenderId={senderId}, @FlowLanguage={lang}").ToListAsync();
        ...
        public async Task<UResponseWithID> AddFlowAsync(int clientId, int userId, FlowDTO obj)
        {
            try
            {
                ...
                var flow = new Flow
                {
                    ...
                    //Status = obj.Status,
                    DataApiVersion = flowDataApiVersionAppSetting != null ? flowDataApiVersionAppSetting.Val : "0",
                    Version = flowVersionAppSetting != null ? flowVersionAppSetting.Val : "0",
